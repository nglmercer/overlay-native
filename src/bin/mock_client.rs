...
fn generate_random_username() -> String {
    let prefixes = [
        "Cool", "Epic", "Super", "Mega", "Ultra", "Pro", "Elite", "Ninja",
        "Shadow", "Dark", "Light", "Fire", "Ice", "Storm", "Thunder",
        "Cyber", "Neon", "Quantum", "Pixel", "Retro", "Crypto", "Meta",
    ];
    
    let suffixes = [
        "Gamer", "Player", "Streamer", "Viewer", "Fan", "User", "Coder",
        "Hacker", "Wizard", "Master", "Lord", "King", "Queen", "Boss",
        "Champion", "Legend", "Hero", "Warrior", "Knight", "Ninja",
    ];
...
fn generate_random_message() -> String {
    let greetings = [
        "Hey everyone!", "Hello!", "Hi chat!", "What's up?", "Greetings!",
        "Yo!", "Hey guys!", "Hi!", "Hello chat!", "Howdy!",
    ];
    
    let reactions = [
        "PogChamp", "LULW", "KEKW", "OMEGALUL", "monkaS", "Sadge",
        "PepeLaugh", "FeelsGoodMan", "FeelsBadMan", "PogU",
    ];
    
    let phrases = [
        "This stream is awesome!", "Great content!", "Love the stream!",
        "Keep up the good work!", "You're amazing!", "Best stream ever!",
        "Let's go!", "Pog moment!", "That was insane!", "CLIP IT!",
        "First time watching, this is great!", "Been here for years!",
        "Just donated!", "Subscribed today!", "Gifted 5 subs!",
        "This game is so fun to watch!", "The gameplay is incredible!",
        "When's the next stream?", "What time zone?", "Love the community!",
        "Chat is so wholesome!", "Best community ever!", "Pogchamp!",
        "That play was clean!", "How did that happen?!", "INSANE!",
        "Skill issue", "Chat, is this real?", "Is this live?", "LOL",
        "LMAO", "ROFL", "haha", "nice one!", "gg", "GG", "Gg",
        "cringe", "based", "W", "L", "W in the chat", "L in the chat",
        "F in chat", "press F", "rip", "oof", "big oof", "massive W",
        "tru", "fr", "no cap", "sheesh", "bussin", "no way",
    ];
    
    let questions = [
        "How long have you been streaming?", "What game is this?",
        "What's your favorite game?", "Do you play any other games?",
        "Can you say hi?", "Can we get some F's in chat?",
        "When is the next subathon?", "How many subs do you have?",
        "What's your rank?", "Any tips for new players?",
        "What's your streaming setup?", "How long have you been playing?",
        "What's your main?", "Favorite streamer?", "Favorite food?",
    ];
    
    let emote_only = [
        "Kappa", "PogChamp", "LUL", "KEKW", "OMEGALUL", "Sadge",
        "PepeHands", "monkaS", "PepeLaugh", "FeelsGoodMan", "FeelsBadMan",
        "PogU", "Pog", "KEKW KEKW", "LUL LUL LUL", "Kappa Kappa Kappa",
    ];
    ...
    let message = match message_type {
        ...
        1 => format!("{} {}", reactions.choose(&mut rng).unwrap(), reactions.choose(&mut rng).unwrap()),
...
fn generate_random_color() -> String {
    let colors = [
        ...
        "#FF9900", "#9900FF", "#00FF99", "#FF0099", "#0099FF",
        ...
        for _ in 0..num_emotes {
            ...
            let pos = if content.contains(name) {
                ...
                EmotePayload {
                    id: id.to_string(),
                    name: name.to_string(),
                    url: Some(format!("https://static-cdn.jtvnw.net/emoticons/v2/{}/default/dark/2.0", id)),
                    is_animated: false,
...
fn generate_random_badges() -> Vec<overlay_native::transport::BadgePayload> {
    ...
    if rng.gen_bool(0.4) {
        ...
        badges.push(overlay_native::transport::BadgePayload {
            ...
            url: Some(format!("https://static-cdn.jtvnw.net/badges/v1/5d9f2208-5dd8-11e7-8513-2ff4adfae661/2")),
    ...
    if rng.gen_bool(0.1) {
        badges.push(overlay_native::transport::BadgePayload {
            ...
            url: Some("https://static-cdn.jtvnw.net/badges/v1/3267646d-33f0-4b17-b3df-f923a41db1d0/2".to_string()),
    ...
    if rng.gen_bool(0.05) {
        badges.push(overlay_native::transport::BadgePayload {
            id: "vip".to_string(),
            name: "VIP".to_string(),
            url: Some("https://static-cdn.jtvnw.net/badges/v1/b817aba4-fad8-49e2-b88a-7cc744f6efff/2".to_string()),
    ...
    if rng.gen_bool(0.02) {
        badges.push(overlay_native::transport::BadgePayload {
            ...
            url: Some("https://static-cdn.jtvnw.net/badges/v1/511b78a9-ab37-472f-9569-457753bbe7d3/2".to_string()),
        ...
        match message {
            IncomingMessage::ChatMessage(payload) => {
                ...
                if !payload.emotes.is_empty() {
                    println!("   😀 Emotes: {}", payload.emotes.iter()
                        .map(|e| e.name.clone())
                        .collect::<Vec<_>>()
                        .join(", "));
                }
                ...
                if !payload.badges.is_empty() {
                    println!("   🏅 Badges: {}", payload.badges.iter()
                        .map(|b| b.name.clone())
                        .collect::<Vec<_>>()
                        .join(", "));
                }
                ...
                println!("   ✅ Converted to core message with ID: {}", core_message.id);
            }
